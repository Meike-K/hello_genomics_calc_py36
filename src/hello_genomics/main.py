...
def main():
    ...
    with genes_path.open('r') as f_in:
        ...
        gene_type_col = header.index("gene_types")
        ...
        for row in reader:
            ...
            # count
            if gene_type == 'unknown' and fg_io.get_parameter('drop_unknown') is True:
                continue
            else:
                gene_types[gene_type] += 1
                total_num_genes += 1

        logger.info(f"Found {total_num_genes} genes and {len(gene_types.keys())} gene types.")
    ...
    results = {'num_genes': total_num_genes, 'gene_types': gene_types}
